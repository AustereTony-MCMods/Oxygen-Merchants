...
public class BondCreationGUICallback extends AbstractGUICallback {
    ...
    public BondCreationGUICallback(ManagementMenuGUIScreen screen, EntitiesManagementGUISection section, int width, int height) {
    ...
    public void handleElementClick(AbstractGUISection section, GUIBaseElement element, int mouseButton) {
        if (mouseButton == 0) { 
            if (element == this.cancelButton)
                ...
                if (this.currentProfile != null) {
                    String 
                    name = this.nameField.getTypedText().isEmpty() ? MerchantsManagerClient.instance().getPointedEntity().getName() : this.nameField.getTypedText(),
                            profession = this.professionField.getTypedText().isEmpty() ? ClientReference.localize("merchants.gui.management.genericProfession") : this.professionField.getTypedText();
                            MerchantsManagerClient.instance().getBoundEntitiesManager().createBondSynced(name, profession, this.currentProfile.id);
