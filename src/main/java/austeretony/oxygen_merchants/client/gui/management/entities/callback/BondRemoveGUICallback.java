...
public class BondRemoveGUICallback extends AbstractGUICallback {
    ...
    public BondRemoveGUICallback(ManagementMenuGUIScreen screen, EntitiesManagementGUISection section, int width, int height) {
    ...
    public void init() {
        ...
        this.addElement(new GUITextLabel(2, 2).setDisplayText(I18n.format("merchants.gui.management.bondRemoveCallback"), true, GUISettings.instance().getTitleScale()));
    ...
    protected void onOpen() {
        this.requestLabel.setDisplayText(I18n.format("merchants.gui.management.bondRemoveCallback.request", 
                MerchantsManagerClient.instance().getBoundEntitiesManager().getBond(this.section.getCurrentBondButton().id).getName()), false, GUISettings.instance().getTextScale());
    }
    ...
    public void handleElementClick(AbstractGUISection section, GUIBaseElement element, int mouseButton) {      
        if (mouseButton == 0) { 
            if (element == this.cancelButton)
                ...
                MerchantsManagerClient.instance().getBoundEntitiesManager().removeBondSynced(this.section.getCurrentBondButton().id);
