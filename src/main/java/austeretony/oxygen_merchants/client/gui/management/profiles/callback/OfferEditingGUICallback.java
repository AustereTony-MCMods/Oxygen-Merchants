...
public class OfferEditingGUICallback extends AbstractGUICallback {
    ...
    private GUICheckBoxButton enableSellButton;
    ...
    protected void onOpen() {
        ...
        this.enableSellButton.setToggled(false);
    ...
    public void handleElementClick(AbstractGUISection section, GUIBaseElement element, int mouseButton) {
        if (mouseButton == 0) { 
            if (element == this.cancelButton)
                ...
                MerchantOffer offer = new MerchantOffer(this.section.getCurrentOfferButton().id, ItemStackWrapper.getFromStack(this.currentButton.getItemStack()));
                offer.setAmount(this.amountField.getTypedNumber());
                offer.setBuyCost(this.buyCostField.getTypedNumber());
                offer.setSellingEnabled(this.enableSellButton.isToggled());
                offer.setSellingCost(this.sellingCostField.getTypedNumber());
                this.section.addOfferToCurrentProfile(offer);
                this.close();
            } else if (element == this.enableSellButton) {
                if (this.enableSellButton.isToggled())
